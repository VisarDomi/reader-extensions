...
<key>EnvironmentVariables</key><dict><key>SIGNING_TEAM</key><string>${xml(signingTeam)}</string></dict>
...
<key>StandardErrorPath</key><string>${xml(remoteRoot)}/gui-build.log</string>
    ...
    execFileSync('rsync', ['-a', '-e', ['ssh', ...sshArgs].map(quote).join(' '),
        resolve(root, 'scripts/build-on-mac.sh'), `${host}:${remoteRoot}/scripts/`], { stdio: 'inherit' });
    ...
    ssh(['set -eu', ...checks, `codesign --verify --deep --strict ${quote(app)}`,
